...
- 구글 로그인, 오너 승인제 가입
- 채널형 단체채팅, 1:1 DM
- 모임 일정 (종류 구분 없이 "모임" 하나), 관리자의 참석자 직접 관리(지난 모임 포함)
- 지도: 주변 보드게임카페 자동 표시, 오너의 즐겨찾기(다녀온 곳 기록)
- 출석 업적 → 경험치 → 티어(브론즈~챌린저), 칭호(자동·관리자 수여), 활동 기록
- 보드게임 라이브러리
- 통계 (모임 참석·출석 기준)
- 공지·게시판
- 회원관리
...
| 기기 | 모바일 기준 반응형 웹, PC에서도 사용. **홈 화면 앱(PWA)으로 설치 가능** (2026-09-19 추가, 스토어 앱은 만들지 않음) |
| 역할 | **오너 / 일반회원** 2단계. 오너 = `kwat09k@gmail.com` (rules에 고정) |
| 생성 권한 | 공지·채팅 채널은 오너만. 모임·게시글은 승인된 회원 누구나. 보드게임 목록은 오너가 파일로 관리 |
...
| 지도 | 카카오맵 JavaScript SDK + 장소 검색("보드카페" 분류). 키는 등록한 도메인에서만 동작 |
...
users/{uid}                        // 공개 프로필: 승인된 회원 전체가 읽음
  ...
  // 경험치·티어·업적은 저장하지 않고 출석 기록으로 매번 계산한다 (src/data/achievements.ts)
...
events/{eventId}
  title, description, location      // 모임은 한 종류 (2026-09-19에 정기모임·번개 구분 없앰. 예전 문서의 type 필드는 무시)
  ...
  attendedIds[]    // 출석 체크. 호스트·오너만 수정 → 출석 통계에 사용
                   // 오너는 attendeeIds·attendedIds 를 직접 바꿀 수 있다 (정원·지난 모임·취소 무관)
  canceled, createdAt
...
places/{카카오 장소 ID}              // 오너가 즐겨찾기한 보드게임 카페 (2026-09-19)
  ...
  // 지도에 보이는 일반 보드게임카페는 저장하지 않고 카카오에서 그때그때 뽑는다
...
| `/` | 홈: **내 프로필 배너(티어·경험치·출석·업적·순위)**, 다음 모임 큰 카드(D-day·참석자 얼굴), 공지·채팅·통계·지도 바로가기 |
...
4. **모임/일정**: 모임(종류 구분 없음), 캘린더, 참석 신청(정원 제한), 출석 체크
...
## 11. 추후 과제 (백로그)
- 형용사 칭호 4개(보드게임에 입문한·보드게임을 사랑하는·예리한·꾸준한): 오너가 조건을 정하면 `src/data/titles.ts`에 추가
- 티어 승급·칭호 획득 팝업 알림 (지금은 본인 프로필 NEW 표시만)
...
- 다크모드
- 플레이스토어 등록 (PWA를 감싸는 TWA 방식, 구글 개발자 등록비 25달러). 아이폰 앱스토어는 맥과 연 99달러가 필요해 보류
