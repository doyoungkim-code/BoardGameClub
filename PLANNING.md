...
places/{placeId}                    // 지도에 찍은 보드게임 카페 (2026-09-19 추가)
  name, address, memo, lat, lng, createdBy, createdAt
  // 등록은 회원 누구나, 수정·삭제는 등록자와 오너. 모임 장소 이름이 같으면 모임 화면에서 지도로 연결
  canceled, createdAt
...
| `/places` | 지도(OpenStreetMap): 카페 핀 찍기·수정·위치 옮기기, 카카오맵 길찾기 연결 |
| `/me` | 닉네임 수정, 로그아웃 |
...
## 11. 추후 과제 (백로그)
- 지도를 카카오맵으로 바꾸기 (한국 지도·장소 검색이 더 좋음. 카카오 개발자 앱 등록과 JavaScript 키, 도메인 등록 필요)
