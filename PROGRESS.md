...
1. **전 기능 실사용 확인** (아직 브라우저로 돌려본 적 없음)
   ...
   - 일정 투표: 모임 탭 "일정 투표" → 후보 날짜 올리기 → 다른 계정으로 투표 → "이 날짜로 모임 만들기" → 투표가 마감되는지
   - 채팅: 띄어쓰기 없는 긴 글·주소가 말풍선 안에서 줄바꿈되는지, 보낸 사람 사진·이름 누르면 프로필로 가는지
   - 게임: 목록 39개와 표지, 검색, "몇 명이서" 필터
   - 게시판: 카테고리(공지는 오너만)를 골라 글쓰기, 댓글, 좋아요, 고정
...
### 게시판 하나 + 카테고리 (2026-09-24)
- 공지·자유·후기로 **나뉘어 있던 게시판을 하나로 합치고**, 글마다 카테고리를 붙여 제목 앞에 태그로 보여준다.
  카테고리: 공지 / 자유 / 보드게임 추천 / 모임 후기 / 질문 (`src/types/post.ts` 의 `POST_CATEGORIES` 에서 고친다)
- **Firestore 필드 이름은 `board` 그대로** 뒀다. 이미 올라간 글(notice/free/review)을 건드리지 않으려고 값만 늘렸다
  (코드에서 타입 이름은 `PostCategory`). 새 값은 `recommend`, `question`
- 화면 주소는 `/board?c=<카테고리>`(전체는 `/board`). 예전 주소 `/board/notice` 는 `features/board/BoardRedirect.tsx` 가 옮겨 준다
- 공지만 오너 전용(작성·고정). 글을 쓴 뒤 **카테고리 변경은 금지**(rules) — 자유글을 공지로 바꿔 권한을 우회하는 걸 막는다
- **복합 색인 1개 추가:** posts(pinned desc, createdAt desc) — 카테고리 없이 전체를 볼 때 쓴다
  (카테고리를 고르면 기존 posts(board, pinned desc, createdAt desc) 색인)

### 채팅을 헤더 아이콘으로 (2026-09-24)
- 하단 탭 다섯 칸 중 채팅 자리를 **게시판**에 주고, 채팅은 화면 위쪽 프로필 사진 옆 아이콘으로 옮겼다
  (`components/layout/nav.ts` 의 `CHAT_ITEM`, 안 읽음 배지 유지). PC 사이드바에는 그대로 메뉴로 있다
- 탭은 기록을 쌓지 않지만(홈으로 뒤로가기), 헤더 채팅 아이콘은 일반 이동이라 뒤로가기로 원래 화면에 돌아온다

### 6~7단계에서 정한 것
- **카테고리 변경 금지:** 글을 쓴 뒤에는 `board`(카테고리)를 바꿀 수 없다(rules). 자유글을 공지로 옮겨
...
- **복합 색인:** posts(board, pinned desc, createdAt desc).
...
### 프로필 배너 (2026-09-23, 프로필 상세까지 확장 2026-09-24)
- 모습은 `features/members/TierBanner.tsx` 하나로 두고 **홈과 회원 프로필 상세가 같이 쓴다**.
  값은 부르는 쪽에서 넣는다: 홈은 캐시된 출석 횟수(출석·업적·순위 3칸), 프로필은 불러온 활동 기록(출석·주최·게시글·순위 4칸)
- 배경색·테두리·방패가 티어 색을 따라가서 티어가 오르면 모습이 바뀐다. 색 섞는 식은 `lib/tier.ts`
  (`tierTint`/`tierEdge`/`tierGradient`, `color-mix` 사용 → 나중에 다크 모드를 켜도 같은 식이 쓰인다)
- 업적 칸과 대표 칭호도 같은 티어 색을 옅게 입힌다. 못 얻은 업적은 점선 + 진행도 막대(티어 색)
- 예전 `TierCard.tsx` 는 배너가 대신하므로 삭제. 구역 제목은 `components/SectionTitle.tsx` 로 통일
- 홈 배너는 누르면 내 프로필로 간다 (`to` 를 주면 링크가 된다)
...
- 표는 문서 안 `votes` 맵에 `uid → 후보 id 목록`으로 저장. rules가 **자기 칸만** 바꾸게 막는다
  (`votes.diff().affectedKeys().hasOnly([uid])`). 하나도 안 고르면 칸을 지운다
- 가장 많이 되는 날짜에 👑 표시. 만든 사람·오너가 **"이 날짜로 모임 만들기"** →
  모임 만들기 화면이 투표 내용·날짜로 채워지고(`/events/new?poll=&option=`), 저장하면
  **모임 생성과 투표 마감을 같은 batch로** 한다 (`createEventFromPoll`). 시간 미정 후보는 저녁 7시로 채움
- 모임 없이 마감·다시 열기·삭제는 만든 사람과 오너. 투표 도중 후보를 고치는 화면은 아직 없음(rules는 허용)
- 모임 탭 목록 맨 위에 진행 중인 투표만 보인다 (`status == 'open'` 한 필드 조건, 색인 불필요)

### 채팅 말풍선 줄바꿈 (2026-09-20)
- `index.css`의 body에 `word-break: keep-all`(한글 단어를 자르지 않음)이 걸려 있어서, 띄어쓰기 없는
  긴 글("ㅋㅋㅋ…", 긴 주소)이 줄을 못 바꾸고 말풍선을 옆으로 늘렸다
- 말풍선에만 `wrap-anywhere`(필요할 때 아무 데서나 줄바꿈)와 `min-w-0`을 걸었다. 다른 곳에서 같은
  증상이 보이면 같은 방법으로 고친다
